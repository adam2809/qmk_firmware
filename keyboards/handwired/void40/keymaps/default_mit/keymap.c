...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
	...
	[_LOWER] = LAYOUT_planck_mit(KC_TRNS, KC_NO,KC_AT,KC_PLUS ,KC_HASH, KC_DLR, KC_CIRC, KC_RPRN, KC_RBRC, KC_RCBR, KC_NO, KC_TRNS, KC_TRNS, KC_NO, KC_EQL, KC_MINS, KC_ASTR, KC_BSLS, KC_PERC, KC_LPRN, KC_LBRC, KC_LCBR, KC_NO, KC_NO, KC_TRNS, KC_NO, KC_AMPR, KC_PIPE, KC_EXLM, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_NO, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_UNDS, KC_TRNS, KC_HOME, KC_PGUP, KC_PGDN, KC_END),
...
layer_state_t state_on_adjust_activation;
layer_state_t layer_state_set_user(layer_state_t state) {
    bool prev_adjust_layer_state = IS_LAYER_ON_STATE(state, _ADJUST);
    state = update_tri_layer_state(state, _LOWER, _RAISE, _ADJUST);
    bool curr_adjust_layer_state = IS_LAYER_ON_STATE(state, _ADJUST);

    if(!prev_adjust_layer_state && curr_adjust_layer_state){
        state_on_adjust_activation = prev_adjust_layer_state;
    }
    if(prev_adjust_layer_state && !curr_adjust_layer_state){
        return state_on_adjust_activation;
    }
